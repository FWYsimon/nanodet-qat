...
from ..loss.gfocal_loss import DistributionFocalLoss, QualityFocalLoss, QATQualityFocalLoss, QATDistributionFocalLoss
from ..loss.iou_loss import GIoULoss, QATGIoULoss
...
class QuantizableLoss(nn.Module):
    def __init__(
        self,
        num_classes,
        loss,
        ...
        **kwargs
        ...
        super(QuantizableLoss, self).__init__()
        ...
        # )
        self.loss_qfl = QATQualityFocalLoss(
            beta=self.loss_cfg.loss_qfl.beta,
            loss_weight=self.loss_cfg.loss_qfl.loss_weight,
        )
        self.loss_dfl = QATDistributionFocalLoss(
            loss_weight=self.loss_cfg.loss_dfl.loss_weight
        )
        self.loss_bbox = QATGIoULoss(loss_weight=self.loss_cfg.loss_bbox.loss_weight)

    def forward(self, preds, gt_meta, aux_preds=None):
        """Compute losses.
        Args:
...
class QuantizableNanoDetPlusHead(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(QuantizableNanoDetPlusHead, self).__init__()
        self.num_classes = num_classes
    ...
    def forward(self, feats: List[torch.Tensor]):
        ...
        for i, (cls_convs, gfl_cls)in enumerate(zip(self.cls_convs, self.gfl_cls)):
            feat = feats[i]
        ...
        # outputs = self.skip_add.cat(outputs, dim=2).permute(0, 2, 1)
        # for i in range(len(outputs)):
        #     # outputs[i] = outputs[i].permute(0,2,1)
        #     outputs[i] = outputs[i].transpose(1,2)
        outputs = self.skip_add.cat(outputs, dim=2)
