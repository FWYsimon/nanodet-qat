...
def my_giou_loss(pred, target, eps=1e-7):
    r"""`Generalized Intersection over Union: A Metric and A Loss for Bounding
    Box Regression <https://arxiv.org/abs/1902.09630>`_.
...
def qat_giou_loss(pred, target, eps, weight=None, reduction:str="mean", avg_factor=None):
    loss = my_giou_loss(pred, target, eps)
...
class QATGIoULoss(nn.Module):
    def __init__(self, eps=1e-6, reduction="mean", loss_weight=1.0):
        super(QATGIoULoss, self).__init__()
        self.eps = eps
        self.reduction = reduction
        self.loss_weight = loss_weight
    ...
    def forward(
        ...
        **kwargs,
        ...
        assert reduction_override in (None, "none", "mean", "sum")
        reduction = reduction_override if reduction_override else self.reduction
        loss = self.loss_weight * qat_giou_loss(
            pred,
            target,
            eps=self.eps,
            weight=weight,
