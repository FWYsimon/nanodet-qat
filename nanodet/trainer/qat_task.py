...
from ..model.head.quantization_nanodet_plus_head import QuantizableLoss

class QATTrainingTask(LightningModule):
    """
    ...
    def __init__(self, cfg, evaluator=None):
        super(QATTrainingTask, self).__init__()
        self.cfg = cfg
        self.model = build_model(cfg.model)
        self.loss = QuantizableLoss(**(cfg.model.arch.head))
        print(self.loss)
    ...
    def quantize(self):
        # self.model_fp = copy.deepcopy(self.model)
        # model_to_quantize = 
        ...
        # self.model.eval()
        # scripted = torch.jit.script(self.model)
        # model_quantized = torch.quantization.convert(self.model)
    ...
    def predict(self, batch, batch_idx=None, dataloader_idx=None):
        ...
        results = self.loss.post_process(preds, batch)
        return results

    def training_step(self, batch, batch_idx):
        batch = self._preprocess_batch_input(batch)
        # preds, loss, loss_states = self.model.forward_train(batch)
        head_out, aux_head_out = self.model.forward_train(batch)
        loss, loss_states = self.loss(head_out, batch, aux_preds=aux_head_out)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.weight_averager is not None:
            ...
            preds, aux_head_out = self.model.forward_train(batch)
            loss, loss_states = self.loss(preds, batch, aux_preds=aux_head_out)
        else:
            # preds, loss, loss_states = self.model.forward_train(batch)
            preds, aux_head_out = self.model.forward_train(batch)
            loss, loss_states = self.loss(preds, batch, aux_preds=aux_head_out)
        ...
        dets = self.loss.post_process(preds, batch)
        return dets
