...
def main(config, model_path, output_path, input_shape=(320, 320)):
    ...
    divisible = 32
    if divisible > 0:
        h = (input_shape[1] + divisible - 1) // divisible * divisible
        w = (input_shape[0] + divisible - 1) // divisible * divisible
        padding_size = [0, w - input_shape[0], 0, h - input_shape[1]]
        dummy_input = F.pad(dummy_input, padding_size, value=0.0)
